...
try:
    from config import RadarConfig, TargetConfig, KalmanFilterConfig, TrackerConfig, SimulationConfig
except ImportError:
    # Fallback to default values if config not available
    class RadarConfig:
        DETECTION_RANGE_M = 350000.0
        MEASUREMENT_NOISE_M = 75.0
        DETECTION_PROBABILITY = 0.98
    class KalmanFilterConfig:
        ACCELERATION_VARIANCE = 5.0
    class TrackerConfig:
        MAX_TRACK_AGE_S = 30.0
        MIN_HITS_TO_CONFIRM = 3
        ASSOCIATION_THRESHOLD_M = 15000.0
    class SimulationConfig:
        TIMESTEP_S = 1.0
        AREA_WIDTH_M = 500000.0
        AREA_HEIGHT_M = 500000.0
...
class Radar:
    ...
    def __init__(self, range_limit: float, measurement_noise: float, 
                 detection_probability: float = 1.0, scan_period: float = 0.1) -> None:
        """
        ...
        for target in targets:
            ...
            if distance <= self.range_limit:
                ...
                if np.random.rand() < self.detection_probability:
                    ...
                    noise = np.random.randn(2) * self.measurement_noise
...
class RadarSimulationArea:
    ...
    def __init__(self, width: float = None, height: float = None) -> None:
        ...
        """
        self.width = width if width is not None else SimulationConfig.AREA_WIDTH_M
        self.height = height if height is not None else SimulationConfig.AREA_HEIGHT_M
        self.targets: List[Target] = []
        # Create radar with configuration parameters
        self.radar = Radar(
            range_limit=RadarConfig.DETECTION_RANGE_M,
            measurement_noise=RadarConfig.MEASUREMENT_NOISE_M,
            detection_probability=RadarConfig.DETECTION_PROBABILITY
        )
        self.tracker = RadarTracker(
            self.radar,
            acceleration_variance=KalmanFilterConfig.ACCELERATION_VARIANCE,
            max_age=TrackerConfig.MAX_TRACK_AGE_S,
            min_hits=TrackerConfig.MIN_HITS_TO_CONFIRM,
            association_threshold=TrackerConfig.ASSOCIATION_THRESHOLD_M
        )
        self.time = 0.0
        self.dt = SimulationConfig.TIMESTEP_S
    ...
    def run_simulation(self, num_steps: int) -> dict:
        ...
        for _ in range(num_steps):
            ...
            measurements = self.radar.detect(self.targets)
            measurements_log.append(measurements)
